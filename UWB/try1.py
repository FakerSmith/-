...
class SimpleCIRCNN(nn.Module):
    def __init__(self, input_length=32, num_classes=2):
        ...
        # 全连接层
        self.fc1 = nn.Linear(64 * 4, 128)  # 64通道 * 4个时间点
...
def main():
    ...
    # 1. 加载数据
    json_path = "20251229_invasion_id_0x723_RX1.json"  # 修改为你的文件路径
    ...
    try:
        ...
        for i in range(len(cir_data)):
            sample = cir_data[i]
            if np.max(np.abs(sample[:16])) > np.max(np.abs(sample[16:])):
                labels[i] = 0
            else:
                labels[i] = 1
    ...
    # 12. 可视化特征图
    print("\n可视化卷积特征...")
    visualize_feature_maps(model, test_dataset[0][0].unsqueeze(0))
